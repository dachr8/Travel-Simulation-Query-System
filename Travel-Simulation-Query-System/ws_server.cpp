...
namespace fdt {
    ...
    json parse_total_transportation_plan(fdt::TotalTransportationPlan plan) {
...
//                std::cout << single_plan.dump() << std::endl;
        ...
        return total_transportation_plan;
    }
...
//         timev = fdt::get_system_timemill();
        ...
        if (function == "submit_passenger_requirement") {
            ...
            if (j["travel_strategy"] == "min_cost") {
                s = minCost;
            } else if (j["travel_strategy"] == "min_time") {
                s = minTime;
            } else if (j["travel_strategy"] == "limited_time") {
                s = limitedTime;
            }
...
//			std::cout << "strategy ok" << std::endl;
            ...
            for (auto it = pass_by_vertex_display_name_json_array.begin();
                 it != pass_by_vertex_display_name_json_array.end(); ++it) {
                pass_by_vertex.emplace_back(*it);
            }

//			std::cout << "pass_by_vertex ok" << std::endl;

            fdt::PassengerRequirement requirement = fdt::PassengerRequirement(j["from"], j["to"], j["start_time"],
                                                                              pass_by_vertex, s, j["total_time_limit"]);
...
//			std::cout << "requirement build success" << std::endl;
            ...
            json total_transportation_plan = parse_total_transportation_plan(plan);
...
//        }
    ...
    int start_ws_server() {
        ...
        echo.on_close = [](std::shared_ptr<WsServer::Connection> connection, int status,
