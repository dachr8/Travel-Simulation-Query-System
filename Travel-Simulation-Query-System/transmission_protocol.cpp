    ...
    std::vector<Edge> get_all_display_edge() {
        ...
        while (iter != timeTable->getCityMap().end()) {
            ...
            while (it != edge_vector.end()) {
                if ((it->get_destination_vertex_display_name() == iter->first &&
                     it->get_source_vertex_display_name() == iter->second.city)
                    || (it->get_destination_vertex_display_name() == iter->second.city &&
                        it->get_source_vertex_display_name() == iter->first)) {
                    flag = 0;
                    break;
                }
                ++it;
        ...
        if (schedule->status.currentStatus == error) {
            ...
            while (iter != schedule->cities.end()) {
                ...
                if (current_time < iter->time[0]) {
                    ...
                    PlanSingleTransportation singlePlan(st, end, iter->time[0] * plus, iter->time[1] * plus,
                                                        iter->transportation);
                    plan.push_back(singlePlan);
